...
class FlipkartHelper
{
    ...
    public function auto_populate($extra_where_arr = array())
    {
        ...
        $where_cond_arr = array("dc_type" => "flipkart", "updated_on <=" => date("Y-m-d H:i:s", time() - (CRON_THRESHOLD_HOURS * 60 * 60)));
        $where_cond_arr = array_merge($where_cond_arr, $extra_where_arr);
        $flipkart_productIds = $model->fetchSelectedData("dc_id, dc_product_unique_code, dc_category_id", TABLE_DAILY_CRON, $where_cond_arr, "updated_on");
        foreach ($flipkart_productIds as $value)
        {
            $product_category_id = $value["dc_category_id"];
            $product_unique_code = $value["dc_product_unique_code"];
