...
//        Defining orderby for product
    ...
    public function get_product_details($product_url_key, $currency_code = CURRENCY_CODE)
    {
        ...
        $product_where_cond = array("product_status" => 1, "category_status" => 1, "product_url_key" => $product_url_key, "product_currency" => $currency_code);
        ...
        if (!empty($product_data))
        {
            ...
            $fetch_fields = "product_id, product_image_url, product_title, product_price_min, product_url_key, product_type, product_currency";
            $related_products = $model->fetchSelectedData($fetch_fields, TABLE_PRODUCTS, array("product_status" => 1, "product_id !=" => $product_data["product_id"], "product_category_id" => $product_data["product_category_id"], "product_currency" => CURRENCY_CODE), "rand()", "rand()", "0,8");
